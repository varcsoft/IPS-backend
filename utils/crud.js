...
const get = async (req, res, next, table, select, where, orderby) => {
    try {
        const data = await crudf.get(table, req.query, select, where, orderby);
...
const getbyid = async (req, res, next, table, select) => {
    try {
        const data = await crudf.getbyid(table, req.params.id, select);
        if (!data) {
...
const deletebyid = async (req, res, next, table) => {
    try {
        ...
        return sendresponse(res, `Record with id ${req.params.id} deleted successfully from ${table}`, 200, req);
    } catch (e) {
        next(e);
    }
}
...
const create = async (req, res, next, table, body, required, where) => {
    try {
        ...
        return sendresponse(res, `Record created successfully in ${table}`, 201, req);
    } catch (e) {
        next(e);
    }
}
...
const createmany = async (req, res, next, table, body, required) => {
    try {
        ...
        return sendresponse(res, `Records created successfully in ${table}`, 201, req);
    } catch (e) {
        next(e);
    }
}
...
const update = async (req, res, next, table, body) => {
    try {
        ...
        return sendresponse(res, `Record with id ${req.params.id} updated successfully in ${table}`, 200, req);
    } catch (e) {
        next(e);
    }
}
